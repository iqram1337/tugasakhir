...
PH2DT_CMD = "D:\QuakeFlow\HypoDD\HYPODD\src\ph2dt ph2dt.inp"
HYPODD_CMD = "D:\QuakeFlow\HypoDD\HYPODD\src\hypoDD hypoDD.inp"
#
# # %%
...
def convert_picks():
    ...
    for i in tqdm(range(len(events))):
        if i % MAXEVENT == MAXEVENT - 1:
            ...
            os.system(f"copy hypoDD.pha hypoDD.inp ph2dt.inp stations_hypoDD.dat .\\tmp_{idx:02d}")
...
def run_hypoDD(data_path):
    ...
    os.system(f"copy {data_path}\\hypoDD.reloc .\\hypoDD_{idx}.reloc")
    os.system(f"copy {data_path}\\dt.ct .\\dt_{idx}.ct")
    os.system(f"copy {data_path}\\event.dat .\\event_{idx}.dat")
    os.system(f"copy {data_path}\\event.sel .\\event_{idx}.sel")
...
if __name__ == "__main__":
    ...
    hypoDD_catalog_str = ' '.join(hypoDD_catalogs)
    print(f"type {hypoDD_catalog_str} > hypoDD_catalog.txt")
    os.system(f"type {hypoDD_catalog_str} > hypoDD_catalog.txt")
